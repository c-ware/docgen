...
do {                                                                        \
    ...
    fprintf(stderr, "%s (%s:%i)\n", #function_name ": argument '" #argument \
            "' cannot be NULL", __FILE__, __LINE__);                        \
    abort();                                                                \
} while(0)
...
do {                                                                          \
    if((argument) >= (start) && (argument) <= end)                            \
        break;                                                                \
                                                                              \
    fprintf(stderr, "%s: argument '%s' (%i) out of range (%i, %i) (%s:%i)\n", \
            #function_name, #argument, (start), (end), argument, __FILE__,    \
            __LINE__);                                                        \
    abort();                                                                  \
} while(0)
...
do {                                                                              \
    ...
    fprintf(stderr, "%s: argument '%s' (%s) produces truncated buffer at "        \
            "runtime. maximum length is %i (%s:%i)\n", #function_name, #argument, \
            argument, length, __FILE__, __LINE__);                                \
    abort();                                                                      \
} while(0)
...
do {                                                                         \
    if((argument) != (value))                                                \
        break;                                                               \
                                                                             \
    fprintf(stderr, "%s: argument '%s' cannot be value " format "(%s:%i)\n", \
            #function_name, #argument, value, __FILE__, __LINE__);           \
    abort();                                                                 \
} while(0)
...
do {                                                                  \
    if((argument) >= 0)                                               \
        break;                                                        \
                                                                      \
    fprintf(stderr, "%s: argument '%s' cannot be negative (%s:%i)\n", \
            #function_name, #argument, __FILE__, __LINE__);           \
    abort();                                                          \
} while(0)
...
do {                                                                  \
    if((argument) <= 0)                                               \
        break;                                                        \
                                                                      \
    fprintf(stderr, "%s: argument '%s' cannot be positive (%s:%i)\n", \
            #function_name, #argument, __FILE__, __LINE__);           \
    abort();                                                          \
} while(0)
